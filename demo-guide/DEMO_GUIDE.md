...
- [ ] GitHub PR showing the [x] failed schema check (Proposals task failing)
...
**Show:** The GitHub PR with the [x] schema-check failing.

Click into the check details:
```
[x] Proposals: 1 change (estimatedDelivery: String on Order) not included in an approved proposal
```
...
Status transitions to: **[ok] APPROVED**
...
**Show:** The check turns [ok] green:
```
[ok] Proposals: All changes included in approved proposals
[ok] Composition: No errors
[ok] Operations: No breaking changes
```
...
**Show in Studio:** Proposal status -> **[ok] IMPLEMENTED**
